...
$highlight_posts = get_field( 'highlight_posts', $args['page_id'] );
$highlight_tag   = get_term_by( 'name', 'highlight', 'post_tag' );
$cta_tag_url     = get_term_link( $highlight_tag );

if ( isset( $highlight_posts ) && ! empty( $highlight_posts ) ) {
	?>
		<div class="what-we-written">
			<div class="container">
				<div class="what-we-written-wrapper">
					<div class="what-we-written-title">
						<h2><?php echo esc_html( 'Highlights' ); ?></h2>
					...
					<div class="what-we-written-list">
						<?php
						foreach ( $highlight_posts as $highlight_post ) {
							$post_id         = $highlight_post;
